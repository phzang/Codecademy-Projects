...
class Franchise:
  ...
  def available_menus(self, time):
    p_time = parser.parse(time)
    available_menus = []

    for m in self.menus:
      if ( (p_time >= parser.parse(m.start_time)) and p_time <= parser.parse(m.end_time)):
...
dinner = Menu("Dinner", {
  ...
  'mushroom ravioli (vegan)': 13.50, 'coffee': 2.00, 'espresso': 3.00, }, "5pm", "11pm")

kids = Menu("Kids", {
  'chicken nuggets': 6.50, 'fusilli with wild mushrooms': 12.00,
  'apple juice': 3.00 }, "11am", "9pm")
